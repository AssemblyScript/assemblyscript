...
export type ASExport = { [key: string]: any }
...
export namespace utils {
    ...
    export function readString(U32: Uint32Array, U16: Uint16Array, ptr: number): string {
        ...
        while (dataRemain > chunkSize) {
            ...
            let part: any = U16.subarray(dataOffset, dataOffset += size)
...
class MemoryWrapper implements ASMemory {
    ...
    static resolve(value: MemoryWrapper | WebAssembly.Memory): MemoryWrapper {
    ...
    newString(str: string): number {
        ...
        let ptr = this.allocate(4 + (dataLength << 1))
        let dataOffset = (4 + ptr) >>> 1
        let U32 = this.U32, U16 = this.U16
    ...
    newArray(viewOrCtor: TypedArray | TypedArrayConstructor, length?: number, unsafe?: boolean): number {
    ...
    copy(dest: number, src: number, n: number) {
        throw new Error('Memory not initialized')
    }

    fill(ptr: number, value: number, size: number) {
        throw new Error('Memory not initialized')
    }
...
class TableWrapper implements ASTable {
    ...
    newFunction(fn: (...args: any[]) => any): number {
        if (typeof (<any>fn)[ORIG_REF] === 'function') fn = (<any>fn)[ORIG_REF]
    ...
    set(index: number, value: (args: any[]) => any) {
            ...
            if (/^(get|set):/.test(name)) {
                ...
                if (!utils.hasOwnProperty(curr, name)) {
                    ...
                    Object.defineProperty(curr, name, {
                        get: function() { return getter(this[SELF_REF]) },
                        set: function(value) { setter(this[SELF_REF], value) },
                        enumerable: true
                    })
                }
            } else {
