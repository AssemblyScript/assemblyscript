...
function test(file) {
  ...
  */
  ...
  // should be able to retain references externally
  {
    let ref = exports.__retain(exports.__newString("test")); // ERC=1
    exports.__release(ref); // ERC=0
    try { exports.__release(ref); assert(false); } catch (e) { /* nop */ }
  }
  
  // should be able to keep references alive externally
  {
    let ref = exports.__keepalive(exports.__newString("test"));
    assert(typeof ref !== "number");
    assert(ref instanceof Number);
    // TODO: somehow verify that finalization happens eventually
  }
