...
function postInstantiate(extendedExports, instance) {
  ...
  function __newArray(id, valuesOrCapacity) {
    const input = valuesOrCapacity;
    const info = getArrayInfo(id);
    const align = getValueAlign(info);
    const arrayLike = input != null && typeof input.length === "number";
    const iterable = input[Symbol.iterator] === 'function';
    const length = arrayLike || iterable ? input.length : +input;
