...
function postInstantiate(extendedExports, instance) {
  ...
  function __newArray(id, valuesOrLen) {
    const info = getArrayInfo(id);
    const align = getValueAlign(info);
    const hasValues = Array.isArray(valuesOrLen);
    const length = hasValues ? valuesOrLen.length : +valuesOrLen;
    ...
    if (hasValues) {
      ...
      if (info & VAL_MANAGED) {
        for (let i = 0; i < length; ++i) {
          const value = valuesOrLen[i];
          view[(buf >>> align) + i] = value;
        }
      } else {
        view.set(valuesOrLen, buf >>> align);
      }
    }
    return result;
  }
