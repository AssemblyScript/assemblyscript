...
function instantiate(module, imports) {
  ...
  /** List of types to use with getArray */
  const TYPES = {
      U32: { name: "U32" },
      I32: { name: "I32" },
      F32: { name: "F32" },
  ...
  function getArray(ptr, type, asTippedArray) {
    ...
    if ([TYPES.I32, TYPES.U32, TYPES.F32].includes(type)) {
      return transform(this[type.name].subarray(dataStart, dataStart + len));
    }
    if (TYPES.STRING === type) {
      return Array.from(U32.subarray(dataStart, dataStart + len)).map((x) => this.getString(x));
    }
    if (TYPES.U64 === type) {
      const mem = Array.from(U32.subarray(dataStart, dataStart + 2 * len));
      const res = [];
      for (let i = 0; i < mem.length; i = i + 2) {
        res.push(mem[i] + TWO_POW_32 * mem[i + 1]);
      }
      return res;
    }
  }
