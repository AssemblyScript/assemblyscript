...
function postInstantiate(extendedExports, instance) {
  ...
  /** Gets the runtime type info for the given id and specific field. */
  function getRtt(id, field = 0) {
    const U32 = new Uint32Array(memory.buffer);
    if ((id >>>= 0) >= getRttCount(U32)) throw Error(`invalid id: ${id}`);
    return U32[(__rtti_base + 4 >>> 2) + (id << 1) + field];
  }

  /** Gets and validate runtime type info for the given id for array like objects */
  function getArrayInfo(id) {
    const info = getRtt(id);
  ...
  function __instanceof(ptr, baseId) {
    ...
    if (id <= getRttCount(U32)) {
      do {
        if (id == baseId) return true;
        id = getRtt(id, 1); // gets runtime base
      } while (id);
    }
    return false;
  }
