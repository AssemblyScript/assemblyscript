...
function instantiate(module, imports) {
  ...
  /** Gets an array from the module's memory by its pointer and its type. */
  function getArray(ptr, type, asTippedArray) {
    checkMem();
    const ptr0 = ptr >>> 2;
    const dataStart = (U32[ptr0] >>> 2) + 2; // 0 -> size; 1 -> static 0
    const len = U32[ptr0 + 1];
    const transform = asTippedArray ? (x) => x : (x) => Array.from(x);
    const recast = asTippedArray ? (x) => type.constructor.from(x) : (x) => x;
    if (type === U16 || type == U8) {
      return recast(U32.subarray(dataStart, dataStart + len));
    }
    if (type === I16 || type == I8) {
      return recast(I32.subarray(dataStart, dataStart + len));
    }
    if (type === String) {
      return Array.from(U32.subarray(dataStart, dataStart + len)).map((x) => this.getString(x));
    }
    if (type === Boolean) {
      return Array.from(U32.subarray(dataStart, dataStart + len)).map((x) => Boolean(x))
    }
    if (type === F64) {
      return transform(type.subarray(dataStart >>> 1, (dataStart >>> 1) + len));
    }
    return transform(type.subarray(dataStart, dataStart + len));
  }
