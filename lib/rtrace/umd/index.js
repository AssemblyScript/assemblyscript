...
var rtrace = (function(exports) {
  ...
  class Rtrace {
    constructor(options) {
      ...
      this.stackBase = 0;
      this.stackSize = 0;
      this.freedBlocks = new Map(); // The following hooks cannot just be on the prototype but must be
    ...
    accessShadow(ptr, size, isLoad) {
      ...
      if (value != 0) return; // FIXME: this is extremely slow
    ...
    getBlockInfo(ptr) {
      ...
      const mmTags = [[], ["FREE"], ["LEFTFREE"], ["FREE", "LEFTFREE"]]; // 2=LEFTFREE, 1=FREE
  
      const gcColor = ["BLACK/WHITE", "WHITE/BLACK", "GRAY"];
      var size = mmInfo & ~3;
      return {
        ptr,
    ...
    onmove(oldPtr, newPtr) {
      ...
      if (!this.blocks.has(oldPtr)) {
        ...
        if (!this.blocks.has(newPtr)) {
          ...
          this.oninfo("MOVE " + oldPtr + ".." + (oldPtr + oldInfo.size) + " -> " + newPtr + ".." + (newPtr + newInfo.size)); // calls new alloc before and old free after
        }
      }
    }
    ...
    store_val_i32(id, value) {
      if (id == this.whatId && value == 0) {
        console.log("what?", new Error().stack);
      }
      return value;
    }
