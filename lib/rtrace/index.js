...
export class Rtrace {
  ...
  plot(total, pause = 0) {
    if (!this.gcDataStart) this.gcDataStart = Date.now();
    this.gcData.push([ Date.now() - this.gcDataStart, total, pause ]);
  }

  oninterrupt(total) {
    this.interruptStart = process.hrtime();
    this.plot(total);
  }

  onyield(total) {
    var taken = process.hrtime(this.interruptStart);
    var millis = taken[0] * 1e3 + taken[1] / 1e6;
    if (millis >= 1) console.log("interrupted for " + millis.toFixed(1) + "ms");
    this.plot(total, millis);
  }
