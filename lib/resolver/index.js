...
class Entry {
  constructor(file, type = SourceType.Local, packageFolder = "") {
    ...
    if (type === SourceType.Local) {
      ...
      const absoluteFilePath = path.join(packageFolder, file);
      this.file = path.basename(absoluteFilePath);
      this.baseDir = path.dirname(absoluteFilePath);
      this.url = null;
    }
  }
  ...
  resolveAsync(readFile, fetch, callback) {
    if (this.type === SourceType.Package) {
      ...
      const moduleFolder = this.baseDir;
      const fileRelativePath = this.file;
      const packageJsonPath = path.join(moduleFolder, );
      let ascFolder = "assembly";
      readFile("package.json", packageJsonPath, (err, pkg) => {
        if (pkg && !err) {
          try {
            const jsonContents = JSON.parse(pkg);
            if (typeof jsonContents.ascMain === "string") {
...
function resolveFromLocal(fromSourcePath, toSourcePath, args) {
  ...
  for (let k = 0; k < fromSourceEntriesLength; k++) {
    const fromSourceFolder = isFromSourcePathAbsolute
      ? (platform === "win32" ? fromSourceEntries[0] + path.sep : path.sep + path.sep)
        + path.join(...fromSourceEntries.slice(1, fromSourceEntriesLength - k))
      : path.join(...fromSourceEntries.slice(0, fromSourceEntriesLength - k));
    ...
    for (let j = 0; j < moduleFolders.length; j++)  {
      ...
      for (let i = 0; i < toSourceEntriesLength; i++) {
        const index = toSourceEntriesLength - 1 - i;
        const toSourceStart = path.join(...toSourceEntries.slice(0, index));
        const packageFolderName = toSourceEntries[index];
        const toSourceEnd = path.join(...toSourceEntries.slice(index + 1));

        // baseDir is the package folder. Not the baseDir.
        const packageFolder = path.join(fromSourceFolder, moduleFolder, toSourceStart, packageFolderName);

        if (packageFolder.startsWith("C:bower")) {
          console.log(packageFolder);
          console.log(fromSourceFolder);
          console.log(fromSourceEntries);
          console.log(k);
        }
