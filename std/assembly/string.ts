      ...
      if (isAscii(c)) {
        ...
        if (c - 0x24D0 <= 0x24E9 - 0x24D0) {
          ...
          if (c - 0x00DF <= 0xFB17 - 0x00DF) {
            index = <usize>bsearch(c, changetype<usize>(specialsUpper), specialsUpperLen);
          }
          if (~index) {
            // load next 3 code points from row with `index` offset for specialsUpper table
            let ab = load<u32>(changetype<usize>(specialsUpper) + (index << 1), 2);
            let cc = load<u16>(changetype<usize>(specialsUpper) + (index << 1), 6);
