...
function ensureCapacity(
  ...
  if (newSize > oldCapacity >>> alignLog2) {
    if (newSize > BLOCK_MAXSIZE >>> alignLog2)
    ...
    if (canGrow)
      newCapacity = max(min(oldCapacity << 1, BLOCK_MAXSIZE), newCapacity);
...
export class Array<T> {
  ...
  static isArray<U>(value: U): bool {
    return isReference<U>()
      ? changetype<usize>(value) != 0 && builtin_isArray(value)
      : false;
  }

  static create<T>(capacity: i32 = 0): Array<T> {
    WARNING(
      "'Array.create' is deprecated. Use 'new Array' instead, making sure initial elements are initialized.",
    );
  ...
  constructor(length: i32 = 0) {
    ...
    let buffer = changetype<ArrayBuffer>(
      __new(bufferSize, idof<ArrayBuffer>()),
    );
    if (ASC_RUNTIME != Runtime.Incremental) {
  ...
  every(fn: (value: T, index: i32, array: Array<T>) => bool): bool {
    for (let i = 0, len = this.length_; i < min(len, this.length_); ++i) {
      if (!fn(load<T>(this.dataStart + ((<usize>i) << alignof<T>())), i, this))
        return false;
    }
    return true;
  }
  ...
  indexOf(value: T, fromIndex: i32 = 0): i32 {
    ...
    let ptr = this.dataStart;
    while (fromIndex < len) {
      if (load<T>(ptr + ((<usize>fromIndex) << alignof<T>())) == value)
        return fromIndex;
      ++fromIndex;
    }
    return -1;
  ...
  push(value: T): i32 {
    ...
    if (isManaged<T>()) {
      store<usize>(
        this.dataStart + ((<usize>oldLen) << alignof<T>()),
        changetype<usize>(value),
      );
      __link(changetype<usize>(this), changetype<usize>(value), true);
    } else {
  ...
  concat(other: Array<T>): Array<T> {
    ...
    let out = changetype<Array<T>>(
      __newArray(outLen, alignof<T>(), idof<Array<T>>()),
    );
  ...
  map<U>(fn: (value: T, index: i32, array: Array<T>) => U): Array<U> {
    ...
    for (let i = 0; i < min(len, this.length_); ++i) {
      let result = fn(
        load<T>(this.dataStart + ((<usize>i) << alignof<T>())),
        i,
        this,
      );
  ...
  shift(): T {
    ...
    let lastIndex = len - 1;
    memory.copy(base, base + sizeof<T>(), (<usize>lastIndex) << alignof<T>());
    if (isReference<T>()) {
  ...
  unshift(value: T): i32 {
    ...
    ensureCapacity(changetype<usize>(this), len, alignof<T>());
    let ptr = this.dataStart;
    memory.copy(ptr + sizeof<T>(), ptr, (<usize>(len - 1)) << alignof<T>());
    store<T>(ptr, value);
    if (isManaged<T>()) {
      __link(changetype<usize>(this), changetype<usize>(value), true);
    }
  ...
  slice(start: i32 = 0, end: i32 = i32.MAX_VALUE): Array<T> {
    ...
    len = max(end - start, 0);
    let slice = changetype<Array<T>>(
      __newArray(len, alignof<T>(), idof<Array<T>>()),
    );
  ...
  splice(start: i32, deleteCount: i32 = i32.MAX_VALUE): Array<T> {
    ...
    let result = changetype<Array<T>>(
      __newArray(deleteCount, alignof<T>(), idof<Array<T>>()),
    );
    let resultStart = result.dataStart;
    let thisStart = this.dataStart;
    let thisBase = thisStart + ((<usize>start) << alignof<T>());
    memory.copy(resultStart, thisBase, (<usize>deleteCount) << alignof<T>());
  ...
  flat(): T {
    ...
    let outBuffer = changetype<ArrayBuffer>(
      __new(byteLength, idof<ArrayBuffer>()),
    );
    ...
    store<i32>(
      changetype<usize>(outArray),
      byteLength,
      offsetof<T>("byteLength"),
    );
    store<usize>(
      changetype<usize>(outArray),
      changetype<usize>(outBuffer),
      offsetof<T>("dataStart"),
    );
    store<usize>(
      changetype<usize>(outArray),
      changetype<usize>(outBuffer),
    ...
    for (let i = 0; i < len; ++i) {
      ...
      let childDataLength =
        (<usize>load<i32>(child, offsetof<T>("length_"))) << align;
      memory.copy(
    ...
    if (isManaged<valueof<T>>()) {
      for (let i = 0; i < size; ++i) {
        let ref = load<usize>(
          changetype<usize>(outBuffer) +
            ((<usize>i) << usize(alignof<valueof<T>>())),
        );
