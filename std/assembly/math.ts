...
export namespace NativeMathf {
  ...
  export function pow(x: f32, y: f32): f32 { // see: musl/src/math/powf.c and SUN COPYRIGHT NOTICE above
    if (ASC_SHRINK_LEVEL < 1) {
      ...
      let hx = reinterpret<i32>(x);
      let hy = reinterpret<i32>(y);
      ...
      let t1: f32, t2: f32, r: f32, s: f32, t: f32, u: f32, v: f32, w: f32, p_h: f32, p_l: f32;
      let n: i32, is: i32;
      if (iy > 0x4D000000) {
      ...
      let y1 = reinterpret<f32>(is & 0xFFFFF000);
      p_l = (y - y1) * t1 + y * t2;
      p_h = y1 * t1;
      z = p_l + p_h;
      j = reinterpret<u32>(z);
