...
export class StaticArray<T> {
  ...
  static fromArray<T>(source: Array<T>): StaticArray<T> {
    ...
    let out = changetype<StaticArray<T>>(
      __new(outSize, idof<StaticArray<T>>()),
    );
    if (isManaged<T>()) {
  ...
  constructor(length: i32) {
    if (<u32>length > (<u32>BLOCK_MAXSIZE) >>> alignof<T>())
  ...
  get length(): i32 {
    return (
      changetype<OBJECT>(changetype<usize>(this) - TOTAL_OVERHEAD).rtSize >>>
      alignof<T>()
    );
  }
  ...
  at(index: i32): T {
    ...
    let value = load<T>(
      changetype<usize>(this) + ((<usize>index) << alignof<T>()),
    );
    if (isReference<T>()) {
      if (!isNullable<T>()) {
  ...
  includes(value: T, fromIndex: i32 = 0): bool {
    if (isFloat<T>()) {
      ...
      while (fromIndex < length) {
        let elem = load<T>(
          changetype<usize>(this) + ((<usize>fromIndex) << alignof<T>()),
        );
        // @ts-ignore
  ...
  concat<U extends ArrayLike<T> = Array<T>>(other: U): U {
    ...
    if (out instanceof Array<T>) {
      ...
      out = changetype<U>(
        __new((<usize>outLen) << alignof<T>(), idof<StaticArray<T>>()),
      );
  ...
  slice<U extends ArrayLike<T> = Array<T>>(
    start: i32 = 0,
    end: i32 = i32.MAX_VALUE,
  ): U {
    let length = this.length;
    ...
    let sourceStart =
      changetype<usize>(this) + ((<usize>start) << alignof<T>());
  ...
  findIndex(fn: (value: T, index: i32, array: StaticArray<T>) => bool): i32 {
    for (let i = 0, len = this.length; i < len; ++i) {
      if (
        fn(
          load<T>(changetype<usize>(this) + ((<usize>i) << alignof<T>())),
          i,
          this,
        )
      )
  ...
  findLastIndex(
    fn: (value: T, index: i32, array: StaticArray<T>) => bool,
  ): i32 {
  ...
  map<U>(fn: (value: T, index: i32, array: StaticArray<T>) => U): Array<U> {
    let len = this.length;
    let out = changetype<Array<U>>(
      __newArray(len, alignof<U>(), idof<Array<U>>()),
    );
  ...
  filter(fn: (value: T, index: i32, array: StaticArray<T>) => bool): Array<T> {
    let result = changetype<Array<T>>(
      __newArray(0, alignof<T>(), idof<Array<T>>()),
    );
    for (let i = 0, len = this.length; i < len; ++i) {
      let value = load<T>(
        changetype<usize>(this) + ((<usize>i) << alignof<T>()),
      );
  ...
  join(separator: string = ","): string {
    if (isBoolean<T>())
      return joinBooleanArray(changetype<usize>(this), this.length, separator);
    if (isInteger<T>())
      return joinIntegerArray<T>(
        changetype<usize>(this),
        this.length,
        separator,
      );
    if (isFloat<T>())
      return joinFloatArray<T>(changetype<usize>(this), this.length, separator);
    if (ASC_SHRINK_LEVEL < 1) {
      if (isString<T>())
        return joinStringArray(changetype<usize>(this), this.length, separator);
    }
    if (isReference<T>())
      return joinReferenceArray<T>(
        changetype<usize>(this),
        this.length,
        separator,
      );
