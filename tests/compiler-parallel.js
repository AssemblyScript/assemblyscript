...
const testsChunks = splitToChunks(tests, numCPUs);
const chunks = {};
const promises = [];
...
if (cluster.isMaster) {
  ...
  // console.log(testsChunks.length);

  // console.log(Object.keys(cluster.workers).length, Object.keys(cluster.workers));

  for (const id in cluster.workers) {
    chunks[id] = testsChunks.shift();
    // console.log("CHUNKS", testsChunks);
    ...
    promises.push(new Promise((resolve) => {
      ...
      cluster.workers[id].send(chunks[id]);
    }));
    ...
    chunks.forEach((filename, j) => {
      ...
      asc.main(cmd, {
        ...
        if (expectStderr) {
          ...
          let lastIndex = 0;
          let failed = false;
          expectStderr.forEach((substr, k) => {
            var index = stderrString.indexOf(substr, lastIndex);
            if (index < 0) {
              console.log("Missing pattern #" + (k + 1) + " '" + substr + "' in stderr at " + lastIndex + "+.");
