...
interface RpcMember { name: string; ref: TypeRef; }
...
function describeCallable(fn: FunctionDeclaration): RpcCallable {
  ...
  for (let i = 0, k = nodes.length; i < k; ++i) {
    let node = nodes[i];
    params.push({ name: node.name.text, ref: describeType(node.type) });
  }
...
function collectClass(cls: ClassDeclaration, surface: RpcSurface): void {
  ...
  if (hasDecorator(decorators, DecoratorKind.Data)) {
    ...
    for (let i = 0, k = members.length; i < k; ++i) {
      ...
      fields.push({ name: field.name.text, ref: describeType(field.type) });
    }
 ...
 * exposed (no `@data`/`@remote`/`@service`).
 */
export function buildServerModule(program: Program): string | null {
  ...
  out += "// Working @data codec + the typed client-callable Server surface. The codec uses\n";
  out += "// the global DataWriter/DataReader; the Server proxy + transport come from toiljs.\n\n";
