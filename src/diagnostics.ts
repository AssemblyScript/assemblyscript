...
export function diagnosticCategoryToString(
...
export function diagnosticCategoryToColor(
  category: DiagnosticCategory,
): string {
  switch (category) {
    case DiagnosticCategory.Pedantic:
  ...
  if (range) {
    ...
    if (relatedRange) {
      ...
      minLine = max(
        ...
        relatedRange.source.lineAt(relatedRange.start),
      );
    }
