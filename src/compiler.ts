  ...
  makeAssignment(
    ...
    switch (target.kind) {
      ...
      case ElementKind.PROPERTY: { // instance property
        ...
        assert(instance.kind == ElementKind.CLASS || instance.kind == ElementKind.INTERFACE);
        let setterInstance = (<Property>target).setterInstance;
        if (!setterInstance) {
          this.error(
            DiagnosticCode.Cannot_assign_to_0_because_it_is_a_constant_or_a_read_only_property,
    ...
    for (let [name, imem] of imems.entries()) {
      ...
      switch (mem.kind) {
        ...
        case ElementKind.PROPERTY_PROTOTYPE: {
          ...
          to = this.resolver.resolveType(iproperty.getterPrototype!.functionTypeNode, iproperty.getterPrototype!, _interface.contextualTypeArguments, ReportMode.REPORT)!;
          break;
        }
        default: {
          // Error
